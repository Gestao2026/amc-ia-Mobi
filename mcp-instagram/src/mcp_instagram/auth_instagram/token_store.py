"""
Armazenamento seguro do access token do Instagram (Camada 2).
...
Este modulo nunca acessa o Instagram, nunca executa OAuth, nunca gera
...
from mcp_instagram.auth_instagram.crypto import TokenCipher, TokenDecryptionError
...
TARGET_NAME = "mcp-instagram:instagram-access-token"
...
class TokenMetadata:
    ...
    # Identificador numerico da conta do Instagram que autorizou, tal
    # como devolvido pela Meta. Nao e segredo (nao da acesso a nada
    # sozinho) e serve para o captador confirmar QUAL conta ficou
    # conectada. Opcional para continuar lendo um payload gravado antes
...
class InMemoryCredentialBackend:
    ...
    autorizacao no Instagram. E a mesma decisao ja tomada para as
...
class TokenStore:
    ...
    def save_access_token(
        self, access_token: str, expires_at: float, user_id: str | None = None
    ) -> None:
        """
        Grava o access token, o expires_at (epoch seconds) e o id da
        conta como um unico segredo serializado.

        O id da conta entra no mesmo blob por simplicidade: ele nao e
        segredo, mas guarda-lo junto evita um segundo alvo no backend de
        credencial e mantem os dois valores sempre coerentes entre si.
        """
        payload = json.dumps(
    ...
    def get_token_metadata(self) -> TokenMetadata | None:
        """
        Devolve os metadados (expires_at e id da conta) do token
        gravado, ou None se nao houver nenhum. Nunca devolve o token.
        """
        payload = self._read_payload()
        if payload is None:
            return None
