...
Cobrem `config.py`, `auth_linkedin/http_transport.py`,
`auth_linkedin/runtime.py` e a integracao no `server.py` (rota de
callback e as tres ferramentas MCP).

NENHUM teste aqui acessa o LinkedIn real, abre navegador ou usa
credencial real:
...
from mcp_linkedin.auth_linkedin.http_transport import DEFAULT_TIMEOUT_SECONDS, HttpxTransport
...
from mcp_linkedin.auth_linkedin.token_store import InMemoryCredentialBackend, TokenStore
...
FAKE_REDIRECT_URI = f"{FAKE_BASE_URL}{LINKEDIN_CALLBACK_PATH}"
...
class FakeHttpTransport:
    """Transporte falso. Nunca acessa rede; devolve ou levanta o que o teste configurar."""
...
def _runtime(transport=None, clock=None, config=None) -> LinkedInOAuthRuntime:
    """Runtime completo com peças reais, exceto o transporte HTTP (sempre falso)."""
    clock = clock or FakeClock()
    return LinkedInOAuthRuntime(
        config=config or resolve_linkedin_config(_env()),
...
def runtime_injetado(request):
    ...
    def _injetar(runtime):
        modulo_servidor._linkedin_runtime = runtime
        runtimes.append(runtime)
        return runtime

    yield _injetar

    modulo_servidor._linkedin_runtime = modulo_servidor._RUNTIME_NAO_RESOLVIDO
...
def test_camada_2_desligada_sem_nenhuma_variavel():
    assert resolve_linkedin_config(env={}) is None


@pytest.mark.parametrize("faltante", ["LINKEDIN_CLIENT_ID", "LINKEDIN_CLIENT_SECRET", "MCP_PUBLIC_BASE_URL"])
def test_camada_2_desligada_com_configuracao_parcial(faltante):
    env = _env()
    del env[faltante]

    assert resolve_linkedin_config(env=env) is None


def test_variaveis_ausentes_lista_apenas_nomes():
    ausentes = missing_linkedin_env_vars(env={"LINKEDIN_CLIENT_ID": FAKE_CLIENT_ID})

    assert ausentes == ["LINKEDIN_CLIENT_SECRET", "MCP_PUBLIC_BASE_URL"]
    assert FAKE_CLIENT_ID not in ausentes


def test_variaveis_ausentes_vazia_quando_tudo_configurado():
    assert missing_linkedin_env_vars(env=_env()) == []
...
def test_variavel_vazia_conta_como_ausente():
    ...
    assert "LINKEDIN_CLIENT_ID" in missing_linkedin_env_vars(env=env)
    assert resolve_linkedin_config(env=env) is None


def test_configuracao_completa_e_resolvida():
    config = resolve_linkedin_config(env=_env())
...
def test_redirect_uri_e_derivado_da_base_publica():
    ...
    assert config.redirect_uri.endswith("/oauth/linkedin/callback")


def test_barra_final_na_base_publica_nao_duplica_no_redirect_uri():
    config = resolve_linkedin_config(env=_env(MCP_PUBLIC_BASE_URL=f"{FAKE_BASE_URL}/"))
...
def test_escopos_padrao_quando_variavel_ausente():
    ...
    assert config.scopes == DEFAULT_LINKEDIN_SCOPES


def test_escopos_podem_ser_configurados():
    config = resolve_linkedin_config(env=_env(LINKEDIN_SCOPES="openid email w_member_social"))
...
def test_escopos_vazios_geram_erro():
    with pytest.raises(InvalidLinkedInConfigError):
        resolve_linkedin_config(env=_env(LINKEDIN_SCOPES="   "))
...
def test_backend_pode_ser_forcado():
    config = resolve_linkedin_config(env=_env(LINKEDIN_TOKEN_STORE_BACKEND="memory"))
...
def test_backend_invalido_gera_erro():
    with pytest.raises(InvalidLinkedInConfigError):
        resolve_linkedin_config(env=_env(LINKEDIN_TOKEN_STORE_BACKEND="banco-inventado"))
...
def test_organization_urn_ainda_nao_e_lido():
    ...
    config = resolve_linkedin_config(env=_env(LINKEDIN_ORGANIZATION_URN="urn:li:organization:FAKE"))
...
def test_config_nao_faz_chamada_de_rede(monkeypatch):
    ...
    resolve_linkedin_config(env=_env())
...
def test_httpx_transport_devolve_status_e_texto():
    ...
    resposta = _HttpxTransportComMock(handler).post(TOKEN_ENDPOINT, {"grant_type": "authorization_code"})
...
def test_httpx_transport_envia_dados_no_corpo_nao_na_url():
    ...
    _HttpxTransportComMock(handler).post(
        TOKEN_ENDPOINT, {"code": FAKE_AUTHORIZATION_CODE, "client_secret": FAKE_CLIENT_SECRET}
    )
...
def test_httpx_transport_converte_timeout_em_transport_error():
    ...
    with pytest.raises(TransportError):
        _HttpxTransportComMock(handler).post(TOKEN_ENDPOINT, {})
...
def test_httpx_transport_nao_expoe_segredo_no_erro():
    ...
    with pytest.raises(TransportError) as excinfo:
        _HttpxTransportComMock(handler).post(
            TOKEN_ENDPOINT, {"client_secret": FAKE_CLIENT_SECRET, "code": FAKE_AUTHORIZATION_CODE}
        )
...
def test_httpx_transport_nao_segue_redirecionamento():
    ...
    resposta = _HttpxTransportComMock(handler).post(TOKEN_ENDPOINT, {"client_secret": FAKE_CLIENT_SECRET})
...
def test_url_de_autorizacao_usa_a_configuracao():
    ...
    assert query["scope"] == [" ".join(DEFAULT_LINKEDIN_SCOPES)]
...
def test_fluxo_completo_envia_as_credenciais_certas_ao_endpoint_de_token():
    ...
    # senao o LinkedIn recusa: mesma fonte (LinkedInConfig).
...
def test_falha_do_linkedin_na_troca_nao_grava_token():
...
def test_ferramenta_iniciar_sem_configuracao_orienta_o_operador(runtime_injetado):
    ...
    resultado = linkedin_oauth_iniciar()
...
def test_ferramenta_iniciar_devolve_a_url(runtime_injetado):
    ...
    assert resultado["url_autorizacao"].startswith("https://www.linkedin.com/oauth/v2/authorization?")
    assert resultado["escopos_solicitados"] == list(DEFAULT_LINKEDIN_SCOPES)
...
def test_ferramenta_status_sem_configuracao(runtime_injetado):
    ...
    assert linkedin_oauth_status()["status"] == "nao_configurado"


def test_ferramenta_status_antes_e_depois_de_autorizar(runtime_injetado):
    runtime = runtime_injetado(_runtime())

    assert linkedin_oauth_status()["status"] == "nao_conectado"

    autorizacao = runtime.start_authorization()
    runtime.handle_callback(CallbackParams(code=FAKE_AUTHORIZATION_CODE, state=autorizacao.state))

    assert linkedin_oauth_status()["status"] == "conectado"
...
def test_ferramenta_status_nunca_devolve_o_token(runtime_injetado):
    ...
    assert FAKE_ACCESS_TOKEN not in json.dumps(linkedin_oauth_status())


def test_ferramenta_mcp_status_reflete_a_conexao(runtime_injetado):
    runtime = runtime_injetado(_runtime())

    assert linkedin_mcp_status()["oauth"] == "configurado"
    assert linkedin_mcp_status()["linkedin"] == "nao_conectado"

    autorizacao = runtime.start_authorization()
    runtime.handle_callback(CallbackParams(code=FAKE_AUTHORIZATION_CODE, state=autorizacao.state))

    assert linkedin_mcp_status()["linkedin"] == "conectado"
...
def test_rota_de_callback_existe():
    ...
    assert LINKEDIN_CALLBACK_PATH in caminhos
...
def test_callback_nao_exige_autorizacao_da_camada_1(runtime_injetado):
    # O LinkedIn nao envia o Bearer da Camada 1; a rota precisa ser
...
def test_callback_sem_state_e_rejeitado(runtime_injetado):
    ...
    with TestClient(app, base_url=_TEST_BASE_URL) as client:
        resposta = client.get(LINKEDIN_CALLBACK_PATH, params={"code": FAKE_AUTHORIZATION_CODE})

    assert resposta.status_code == 400
    assert resposta.json()["motivo"] == CallbackOutcome.REJECTED_STATE_MISSING.value


def test_callback_com_erro_do_linkedin_e_reportado(runtime_injetado):
    runtime = runtime_injetado(_runtime())
    autorizacao = runtime.start_authorization()
    app = modulo_servidor.mcp.streamable_http_app()
    ...
    assert resposta.status_code == 400
    assert resposta.json()["erro_linkedin"] == "user_cancelled_login"
    assert runtime.token_store.get_access_token() is None
...
def test_callback_sem_configuracao_responde_503(runtime_injetado):
    ...
    with TestClient(app, base_url=_TEST_BASE_URL) as client:
        resposta = client.get(LINKEDIN_CALLBACK_PATH, params={"code": "x", "state": "y"})
...
def test_fluxo_de_ponta_a_ponta_pela_ferramenta_e_pela_rota(runtime_injetado, monkeypatch):
    ...
    devolve a URL, o LinkedIn (aqui, o TestClient) chama o callback com
    ...
    with TestClient(app, base_url=_TEST_BASE_URL) as client:
        resposta = client.get(
            LINKEDIN_CALLBACK_PATH, params={"code": FAKE_AUTHORIZATION_CODE, "state": state}
        )
...
def test_servidor_sobe_sem_a_camada_2_configurada():
    assert resolve_linkedin_runtime(env={}) is None
