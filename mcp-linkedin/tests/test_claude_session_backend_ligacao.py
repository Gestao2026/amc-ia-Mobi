...
from mcp_linkedin.auth_linkedin.crypto import generate_key_base64
...
FAKE_BASE_URL = "https://fake-linkedin.invalid"
...
def _env_supabase(**overrides) -> dict:
    env = _env(**overrides)
    env["LINKEDIN_TOKEN_STORE_BACKEND"] = "supabase"
    env["MCP_LINKEDIN_SUPABASE_URL"] = FAKE_SUPABASE_URL
    env["MCP_LINKEDIN_SUPABASE_KEY"] = FAKE_SUPABASE_KEY
    return env
...
def test_sem_a_camada_2_a_sessao_segue_so_em_memoria():
    env = _env(**{"LINKEDIN_CLIENT_ID": None})
...
def test_backend_que_nao_sobrevive_ao_reinicio_e_recusado(backend):
    ...
    """
    env = _env(**{"LINKEDIN_TOKEN_STORE_BACKEND": backend, VARIAVEL_PONTE_SESSAO: "1"})
...
def test_configuracao_invalida_nao_derruba_a_camada_1():
    """
    Chave de cifragem malformada faz resolve_linkedin_config levantar.
    A Camada 1 precisa subir assim mesmo, sem persistencia.
    """
    env = _env(
        **{"LINKEDIN_TOKEN_ENCRYPTION_KEY": "FAKE_CHAVE_CURTA", VARIAVEL_PONTE_SESSAO: "1"}
    )
