"""
Testes do mcp-instagram: a ferramenta de teste `instagram_mcp_status`,
...
Nenhuma chamada externa, nenhum OAuth, nenhum acesso ao Instagram ou
ao Claude.
"""
...
FAKE_INSTAGRAM_CLIENT_SECRET = "FAKE_CLIENT_SECRET_NAO_REAL"


def test_instagram_mcp_status_retorna_campos_esperados(monkeypatch):
    ...
    # test_instagram.py para o cenario com a Camada 2 ligada.
    monkeypatch.setattr("mcp_instagram.server._instagram_runtime", None)

    resultado = instagram_mcp_status()

    assert resultado["componente"] == "mcp-instagram"
    assert resultado["instagram"] == "nao_conectado"
...
def test_nenhum_segredo_aparece_na_mensagem_de_erro():
    env_com_segredo_irrelevante = {
        ...
        "PORT": "nao-e-numero",
        "INSTAGRAM_CLIENT_SECRET": FAKE_INSTAGRAM_CLIENT_SECRET,
    }

    with pytest.raises(InvalidTransportConfigError) as excinfo:
        resolve_run_config(env=env_com_segredo_irrelevante)

    assert FAKE_INSTAGRAM_CLIENT_SECRET not in str(excinfo.value)
...
def test_resolve_run_config_nao_le_variaveis_de_segredo():
    ...
    assert FAKE_INSTAGRAM_CLIENT_SECRET not in repr(config)
...
def test_requisicao_ao_mcp_recebe_resposta_http_valida_do_servidor():
    ...
    assert corpo["result"]["serverInfo"]["name"] == "mcp-instagram"


def test_servidor_nao_depende_do_instagram():
    # A partir da Etapa 7B, server.py PASSA a importar auth_instagram
    # (a Camada 2 vive nele). O que continua valendo, e e o que este
    # teste protege, e que nada disso e necessario para o servidor
    # funcionar: sem as variaveis do Instagram no ambiente, o runtime
    # resolve para None e o handshake HTTP segue normal. Nenhuma
    # ferramenta de negocio do Instagram (instagram_client) existe ainda.
    import mcp_instagram.server as modulo_servidor

    assert "instagram_client" not in modulo_servidor.__dict__
    assert modulo_servidor.resolve_instagram_runtime(env={}) is None
...
# =====================================================================
# OAuth Claude <-> mcp-instagram, Camada 1 (Etapa 7A)
# =====================================================================
#
...
#
# Nenhum destes testes acessa o Instagram ou o Claude.ai de verdade.
...
def test_caminho_real_de_producao_resulta_em_app_autenticado(monkeypatch, _mcp_auth_state_restaurado):
    ...
    monkeypatch.setenv("MCP_PUBLIC_BASE_URL", "https://mcp-instagram-run7.onrender.com")
    ...
    with TestClient(app, base_url=_MCP_TEST_BASE_URL) as client:
        ...
        assert resposta_as.json()["issuer"] == "https://mcp-instagram-run7.onrender.com"

        resposta_pr = client.get(PROTECTED_RESOURCE_METADATA_PATH)
        assert resposta_pr.status_code == 200
        assert resposta_pr.json()["resource"] == "https://mcp-instagram-run7.onrender.com/mcp"
