...
from mcp_instagram.auth_instagram.crypto import TokenCipher, generate_key_base64
from mcp_instagram.auth_instagram.ponte_backend import (
    CABECALHO_SEGREDO,
...
def test_quem_tem_o_banco_da_ponte_nao_consegue_ler_o_token():
    ...
    from mcp_instagram.auth_instagram.crypto import TokenDecryptionError
...
def test_configuracao_da_ponte_e_resolvida():
    config = resolve_instagram_config(env=_env_ponte())
...
@pytest.mark.parametrize(
    "faltante", ["MCP_INSTAGRAM_PONTE_URL", "MCP_INSTAGRAM_PONTE_SECRET", "INSTAGRAM_TOKEN_ENCRYPTION_KEY"]
)
...
def test_url_sem_https_e_recusada(url):
    ...
    with pytest.raises(InvalidInstagramConfigError) as excinfo:
        resolve_instagram_config(env=_env_ponte(MCP_INSTAGRAM_PONTE_URL=url))

    assert "https" in str(excinfo.value).lower()


def test_https_maiusculo_e_aceito():
    config = resolve_instagram_config(env=_env_ponte(MCP_INSTAGRAM_PONTE_URL="HTTPS://ponte.invalid/t.php"))
...
def test_chave_de_cifragem_invalida_falha_na_inicializacao():
    with pytest.raises(InvalidInstagramConfigError):
        resolve_instagram_config(env=_env_ponte(INSTAGRAM_TOKEN_ENCRYPTION_KEY="chave-curta"))
...
def test_senha_do_mysql_nao_e_variavel_do_render():
    ...
    from mcp_instagram.config import REQUIRED_PONTE_ENV_VARS
...
def test_runtime_monta_backend_cifrado_sobre_a_ponte(monkeypatch):
    ...
    backend = build_credential_backend(resolve_instagram_config(env=_env_ponte()))
...
def test_backend_supabase_continua_disponivel():
    ...
    from mcp_instagram.auth_instagram.supabase_backend import SupabaseCredentialBackend
    from mcp_instagram.config import TOKEN_STORE_BACKEND_SUPABASE, VALID_TOKEN_STORE_BACKENDS
