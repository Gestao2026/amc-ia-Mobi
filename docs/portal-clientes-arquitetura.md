...
5. ~~Pacote 2.~~ Aplicado em 14/09 às 13h05, depois de a captadora comprar créditos, e conferido por SELECT e pela diferença entre versões (`docs/portal-clientes-pacote-2-blindagem.md`). O Lovable retirou UPDATE de `organizacoes`, DELETE de `convites` e DELETE de `config_notificacoes` por não estarem no código. Teste de comportamento feito no mesmo dia, com o OK dela, e todos os itens passaram. Logo depois, também aplicado e conferido às 13h28, com a aprovação do plano no editor, `docs/portal-clientes-nome-da-organizacao.md` (nome da organização editável, antecipado do pacote 11 a pedido da captadora em 14/09), com a permissão de edição de volta; falta só o teste na tela, feito por ela. Próximo: pacote 3, registro no servidor. Seguir assim com cada pacote.
