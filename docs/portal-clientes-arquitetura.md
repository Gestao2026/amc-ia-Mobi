...
5. ~~Pacote 2.~~ Aplicado em 14/09 às 13h05, depois de a captadora comprar créditos, e conferido por SELECT e pela diferença entre versões (`docs/portal-clientes-pacote-2-blindagem.md`). O Lovable retirou UPDATE de `organizacoes`, DELETE de `convites` e DELETE de `config_notificacoes` por não estarem no código. Falta o teste de comportamento, que pede OK. Próximo: `docs/portal-clientes-nome-da-organizacao.md` (nome da organização editável, antecipado do pacote 11 a pedido da captadora em 14/09). Seguir assim com cada pacote.
