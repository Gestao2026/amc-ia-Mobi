...
NENHUM teste aqui acessa o Supabase real, o Instagram ou qualquer rede:
...
from mcp_instagram.auth_instagram.crypto import (
    KEY_SIZE_BYTES,
    NONCE_SIZE_BYTES,
...
from mcp_instagram.auth_instagram.runtime import build_credential_backend
...
def _env_supabase(**overrides) -> dict:
    env = {
        ...
        "INSTAGRAM_TOKEN_STORE_BACKEND": "supabase",
        "MCP_INSTAGRAM_SUPABASE_URL": FAKE_SUPABASE_URL,
        "MCP_INSTAGRAM_SUPABASE_KEY": FAKE_SUPABASE_KEY,
        "INSTAGRAM_TOKEN_ENCRYPTION_KEY": _chave(),
    }
...
def test_leitura_filtra_pelo_alvo_certo():
    ...
    assert "mcp-instagram%3Ainstagram-access-token" in capturado["url"]
...
def test_configuracao_supabase_completa_e_resolvida():
    config = resolve_instagram_config(env=_env_supabase())
...
@pytest.mark.parametrize(
    "faltante",
    ["MCP_INSTAGRAM_SUPABASE_URL", "MCP_INSTAGRAM_SUPABASE_KEY", "INSTAGRAM_TOKEN_ENCRYPTION_KEY"],
)
def test_supabase_sem_variavel_obrigatoria_falha_na_inicializacao(faltante):
    ...
    with pytest.raises(InvalidInstagramConfigError) as excinfo:
        resolve_instagram_config(env=env)
...
def test_chave_de_cifragem_invalida_falha_na_inicializacao():
    ...
    # uma autorizacao no Instagram, nao na hora de gravar o token.
    with pytest.raises(InvalidInstagramConfigError):
        resolve_instagram_config(env=_env_supabase(INSTAGRAM_TOKEN_ENCRYPTION_KEY="chave-curta"))


def test_erro_de_configuracao_nao_expoe_valor_de_credencial():
    env = _env_supabase(INSTAGRAM_TOKEN_ENCRYPTION_KEY="nao-e-base64-!!!")
...
def test_tabela_pode_ser_configurada():
    config = resolve_instagram_config(env=_env_supabase(MCP_INSTAGRAM_SUPABASE_TABLE="tokens_mobi"))
    ...
    with pytest.raises(InvalidInstagramConfigError) as excinfo:
        resolve_instagram_config(
            env={
                ...
                "INSTAGRAM_CLIENT_SECRET": FAKE_CLIENT_SECRET,
                "MCP_PUBLIC_BASE_URL": FAKE_BASE_URL,
                "INSTAGRAM_TOKEN_STORE_BACKEND": "windows",
            }
        )

    assert "ponte" in str(excinfo.value)
...
def test_runtime_monta_backend_cifrado_sobre_supabase(monkeypatch):
    ...
    backend = build_credential_backend(resolve_instagram_config(env=_env_supabase()))
